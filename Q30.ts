...
let usersName = ["Ali", "Akbar", "Haider", "Hamza", "Admin"];

for (let name in usersName) {
  if (usersName[name] == "Admin") {
    console.log(
      `Hello ${usersName[name]}, would you like to see a status report?`
    );
  } else {
    console.log(` Hello ${usersName[name]}, thank you for logging in again.`);
  }
}